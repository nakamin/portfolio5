...
def get_valdata(val_idx, val_subset, device, model):
    ...
    for idx in val_idx:
        image_tensor, true_label = val_subset[idx]
        if true_label in correct_samples:
            continue # 正解画像を記録していたらスキップ

        # 元画像を読み込み
        image_id_full = val_subset.dataset.image_id[val_subset.indices[idx]]
        image_file = image_id_full if image_id_full.endswith('.jpg') else image_id_full + ".jpg"
...
def get_heatmap(cfg, val_idx, val_subset, device, model):

    correct_samples = get_valdata(val_idx, val_subset, device, model)
    ...
    for class_id, (pil_image, true_label, pred_label) in correct_samples.items():   
        ...
        grad_cam = GradCam(model, cfg.train.target_layer) # インスタンス生成
        ...
        return heatmap, correct_samples

def visualize_heatmap(correct_samples, heatmap):
        
    class_id, (pil_img, true_label, pred_label) = correct_samples.items()
